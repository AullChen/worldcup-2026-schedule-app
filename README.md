...
- 自动与本地缓存：首次打开会自动读取 GitHub Pages 发布的最新静态比分；同步结果、预测比分、淘汰赛候选选择和页面偏好会写入 `localStorage`，二次打开优先读取本地缓存。
...
- `What if...` 全赛程模拟：按六大区直接选择 46 支晋级队，再选择 2 支洲际附加赛晋级队，继续模拟 72 场世界杯小组赛与 32 场淘汰赛；上游选择或比分改变后会清理失效的后续结果。
- 模拟冠军庆祝：决赛产生冠军后自动展示奖杯、国旗、礼花与烟火动效，并可从冠军结果区再次播放。
- 小组赛视图：支持按比赛时间或按分组排列。
...
“如果……”中的预选赛采用直接选择方式：保留六大洲足联与各区晋级名额，不再用固定两两对阵决定名额，因此同一区的多支强队可以同时晋级。世界杯小组赛和淘汰赛阶段仍按 48 队、12 组和 32 队淘汰赛推进。

模拟器使用的本地国旗 SVG 来自 MIT 许可的 [flag-icons](https://github.com/lipis/flag-icons)，完整归属信息见 `THIRD_PARTY_NOTICES.md`。
...
- `LICENSE`：MIT License。
- `THIRD_PARTY_NOTICES.md`：第三方开源素材归属。
